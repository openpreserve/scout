...
public class ScoutManager {
	...
	private static final ConfigUtils CONFIG = new ConfigUtils("Knowledge Base");
	...
	private void initDB() {
		...
		final String datafolder = CONFIG
				.getStringProperty(ConfigUtils.KB_DATA_FOLDER_KEY);
		final boolean initdata = CONFIG
				.getBooleanProperty(ConfigUtils.KB_INSERT_TEST_DATA);
	...
	public void start() {
		...
		final Map<String, AdaptorPluginInterface> activeAdaptors = adaptorManager
				.getActiveAdaptorPlugins();

		/**
		 * TEST DATA try { saveTestRequest(manager);
		 * manager.reloadKnownAdaptors(); } catch (final Throwable e) {
		...
		final AssessmentService assessmentService = new AssessmentService(
				notificationService);
		final RequestToDataBinder requestToDataBinder = new RequestToDataBinder(
				assessmentService);

		// Register all notification plugins in notification service
		final List<PluginInfo> notificationPluginInfos = pluginManager
				.getPluginInfo(PluginType.NOTIFICATION);
		...
		for (PluginInfo notificationPluginInfo : notificationPluginInfos) {
			final NotificationPluginInterface notificationPlugin = (NotificationPluginInterface) pluginManager
					.getPlugin(notificationPluginInfo.getClassName(),
							notificationPluginInfo.getVersion());
			...
			public void onChange(ScoutChangeEvent evt) {
				...
				if (message instanceof PluginInfo) {
					...
					if (pluginInfo.getType().equals(PluginType.NOTIFICATION)) {
						final NotificationPluginInterface notificationPlugin = (NotificationPluginInterface) pluginManager
								.getPlugin(pluginInfo.getClassName(),
		...
		final AdaptorsFetchedDataListener resultListener = new AdaptorsFetchedDataListener(
				adaptorManager, dataMerger);
		scheduler.addAdaptorListener(resultListener);

		final AdaptorsLifecycleListener schedulerListener = new AdaptorsLifecycleListener(
				adaptorManager);
		scheduler.addSchedulerListener(schedulerListener);

		// Schedule active adaptors
		for (AdaptorPluginInterface adaptor : activeAdaptors.values()) {
			scheduler.startAdaptor(adaptor, new SourceAdaptorEvent(
					SourceAdaptorEventType.STARTED, "Application startup"));
		}
		...
		DAO.addDOListener(SourceAdaptor.class, new DOListener<SourceAdaptor>() {
			...
			public void onUpdated(SourceAdaptor adaptor) {
				AdaptorPluginInterface adaptorPluginInstance = adaptorManager
						.findAdaptorPluginInstance(adaptor.getInstance());

				if (adaptorPluginInstance == null) {
					// new adaptor
					adaptorPluginInstance = adaptorManager
							.createAdaptorInstance(adaptor.getInstance());
					if (adaptor.isActive()) {
						scheduler.startAdaptor(adaptorPluginInstance,
								new SourceAdaptorEvent(
										SourceAdaptorEventType.STARTED,
					...
					if (adaptor.isActive()) {
						...
						scheduler.stopAdaptor(adaptorPluginInstance,
								new SourceAdaptorEvent(
										SourceAdaptorEventType.STOPPED,
			...
			public void onRemoved(SourceAdaptor adaptor) {
				final AdaptorPluginInterface adaptorInstance = adaptorManager
						.findAdaptorPluginInstance(adaptor.getInstance());
				scheduler.stopAdaptor(adaptorInstance, new SourceAdaptorEvent(
						SourceAdaptorEventType.STOPPED,
		...
		List<QuestionTemplate> templates = DAO.QUESTION_TEMPLATE.list(0,
				DAO.QUESTION_TEMPLATE.count(""));
		...
		parameters0
				.add(new QuestionTemplateParameter(
						"collection",
						"Collection",
						"Your collection profile already inserted into scout",
						ParameterType.NODE,
						"?s watch:type ?type . ?type watch:id \"SD5Mc73YuwE9fH0b8s-h-mzfbvA\"^^xsd:string",
						RequestTarget.ENTITY, null, null));
		final QuestionTemplate template0 = new QuestionTemplate(title0,
				description0, sparql0, target0, parameters0);
		DAO.save(template0);
		...
		parameters1
				.add(new QuestionTemplateParameter(
						...
						"?s watch:type ?type . ?type watch:id \"SD5Mc73YuwE9fH0b8s-h-mzfbvA\"^^xsd:string",
						RequestTarget.ENTITY, null, null));
		parameters1.add(new QuestionTemplateParameter("threshold",
				"Storage size threshold",
				"The storage size above which to raise the alert",
				ParameterType.LITERAL, null, null, DataType.LONG,
				RenderingHint.STORAGE_VOLUME));
		final QuestionTemplate template1 = new QuestionTemplate(title1,
				description1, sparql1, target1, parameters1);
		DAO.save(template1);
	...
	private void addRequest(final AsyncRequest request,
			final RequestToDataBinder requestToDataBinder) {
	...
	private void removeRequest(final AsyncRequest request,
			final RequestToDataBinder requestToDataBinder) {
	...
	public void stop() {
		if (adaptorManager != null && scheduler != null) {
			...
			final int requestCount = DAO.ASYNC_REQUEST.count("");
			List<AsyncRequest> requests = DAO.ASYNC_REQUEST.list(0,
					requestCount);
