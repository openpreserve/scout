...
public class CoreRestTest extends JerseyTest {
  ...
  public static void before() {
    ...
    try {
      ...
      Process exec = Runtime.getRuntime().exec("./4store-testing.sh restart");
      exec.waitFor();
    } catch (IOException e) {
  ...
  /*
   * Uncomment the annotation to remove the 4store kb and undeploy 4store after
   * all tests finish. Otherwise this will be done upon next test run.
   */
  // @AfterClass
  public static void after() {
    try {
      LOG.info("tearing down 4store instance");
      Process exec = Runtime.getRuntime().exec("./4store-testing.sh stop");
      exec.waitFor();
    } catch (IOException e) {
      Assert.fail(e.getMessage());
    } catch (InterruptedException e) {
  ...
  public void entityTypeJSON() {
    ...
    EntityType entitytype2 = client.createEntityType(entitytype);

    System.out.println("Created: " + entitytype2);

    assertEquals(entitytype, entitytype2);
    ...
    Assert.assertTrue(list.contains(entitytype2));
