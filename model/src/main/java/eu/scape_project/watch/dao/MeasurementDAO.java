...
public final class MeasurementDAO extends AbstractDO<Measurement> {
	 ...
	 */
	public Measurement findById(final String entityName,
			final String propertyName, final Date timestamp) {
		return super.findById(
				Measurement.createId(entityName, propertyName, timestamp),
				Measurement.class);
	}
	 ...
	 */
	public List<Measurement> query(final String bindings, final int start,
			final int max) {
	 ...
	 */
	public List<Measurement> query(final String bindings, final int start,
			final int max, final String orderBy) {
		return super.query(Measurement.class, bindings, new QuerySolutionMap(),
				start, max, orderBy);
	}
	 ...
	 * @param bindings
	 *            The query bindings, see
	 *            {@link AbstractDO#count(Class, String)}
	 ...
	 */
	private static String getListByPropertyValueQueryString(
			final PropertyValue value, final boolean showSignificantOnly) {
		String bindings;
		if (showSignificantOnly) {
			bindings = "?s watch:propertyValue "
					+ PropertyValueDAO.getPropertyValueRDFId(value)
	 ...
	 * 
	 * @param value
	 *            The property value from which we want all related
	 *            measurements.
	 * @param showSignificantOnly
	 *            If <code>true</code> only show significant measurements,
	 *            otherwise show all.
	 * @param start
	 ...
	 *         constraints.
	 */
	public List<Measurement> listByPropertyValue(final PropertyValue value,
			final boolean showSignificantOnly, final int start, final int max) {
		return query(
				getListByPropertyValueQueryString(value, showSignificantOnly),
				start, max);
	}
	 ...
	 * @return The query string.
	 */
	private static String getListByAdaptorQueryString(
			final SourceAdaptor adaptor) {
		return "?s watch:adaptor "
				+ SourceAdaptorDAO.getSourceAdaptorRDFId(adaptor);
	}
	 ...
	 * 
	 * @param adaptor
	 *            The source adaptor from which we want all related
	 *            measurements.
	 * @param start
	 *            The start index from which the return list should start.
	 * @param max
	 ...
	 */
	public List<Measurement> listByAdaptor(final SourceAdaptor adaptor,
			final int start, final int max) {
	...
	private static String getListBySourceQueryString(final Source source) {
		return "?s watch:adaptor ?adaptor . ?adaptor watch:source "
				+ SourceDAO.getSourceRDFId(source);
	}
	 ...
	 * @param max
	 *            The maximum number of items to return.
	 * @return A list of the measurements taken on this source, filtered by
	 *         above constraints.
	 */
	public List<Measurement> listBySource(final Source source, final int start,
			final int max) {
	 ...
	 *         constraints.
	 */
	public List<Measurement> listByProperty(final Property property,
			final int start, final int max) {
	 ...
	 * @return The query string.
	 */
	private static String getBindingsByEntityAndProperty(final String entityId,
			final String propertyId, final boolean showSignificantOnly,
			boolean exposeTimestamp) {
		String bindings = "?s watch:propertyValue ?value . ?value watch:property "
				+ PropertyDAO.getPropertyRDFId(propertyId)
				+ " . ?value watch:entity "
				+ EntityDAO.getEntityRDFId(entityId);
		if (showSignificantOnly) {
			bindings += " . ?s watch:significant true";
		}

		if (exposeTimestamp) {
			bindings += " . ?s watch:timestamp ?timestamp";
		}
		return bindings;
	}

	public List<Measurement> listByEntityAndProperty(final String entityId,
			final String propertyId, final boolean showSignificantOnly,
			final int start, final int max) {
		return query(
				getBindingsByEntityAndProperty(entityId, propertyId,
						showSignificantOnly, true), start, max,
				"ASC(?timestamp)");
	}

	public int countByEntityAndProperty(final String entityId,
			final String propertyId, final boolean showSignificantOnly) {
		return count(getBindingsByEntityAndProperty(entityId, propertyId,
	...
	public Measurement findLastMeasurement(final PropertyValue pv) {
		...
		final String sparql = "?s watch:timestamp ?timestamp . ?s watch:propertyValue ?value . ?value watch:property "
				+ PropertyDAO.getPropertyRDFId(pv.getProperty())
				+ " . ?value watch:entity "
