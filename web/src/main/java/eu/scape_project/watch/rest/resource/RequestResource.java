...
public class RequestResource {
	...
	public Response getRequestList(
			...
			@ApiParam(value = "Request query", required = true) @QueryParam("query") String query,
		...
		// XXX Jersey is not correctly decoding some HTML entities from the URL
		// parameter
		query = query.replace("&quot;", "\"");

		LOG.info(
				"Making request list query='{}', bindings={}, target={}, start={}, max={}",
				new Object[] { query, queryBindings, target, start, max });

		if (StringUtils.isBlank(target) || start < 0 || max <= 0) {
			LOG.error("Bad request");
			return Response.status(Status.BAD_REQUEST).build();
		}

		final RequestTarget requestTarget = RequestTarget.valueOf(target
				.toUpperCase());
		final QuerySolutionMap bindings = DAO.QUESTION_TEMPLATE_PARAMETER
				.parseBindings(queryBindings);
		final List<? extends RdfBean<?>> list = DAO.REQUEST.query(
				requestTarget, query, bindings, start, max);

		LOG.info("Request list response: {}", list);
		...
		case ENTITY_TYPE:
			ret = Response
					.ok()
					.entity(new GenericEntity<List<EntityType>>(
		...
		case PROPERTY:
			ret = Response
					.ok()
					.entity(new GenericEntity<List<Property>>(
							(List<Property>) list) {
					}).build();
			break;
		case ENTITY:
			ret = Response
					.ok()
					.entity(new GenericEntity<List<Entity>>((List<Entity>) list) {
					}).build();
			break;
		case PROPERTY_VALUE:
			ret = Response
					.ok()
					.entity(new GenericEntity<List<PropertyValue>>(
		...
		case MEASUREMENT:
			ret = Response
					.ok()
					.entity(new GenericEntity<List<Measurement>>(
	...
	public Response getRequestCount(
		...
		// parameter
		query = query.replace("&quot;", "\"");
		LOG.info("Making request count query='{}', bindings={}, target={}",
				new Object[] { query, queryBindings, target });
