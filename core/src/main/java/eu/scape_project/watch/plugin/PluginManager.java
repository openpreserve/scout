...
public final class PluginManager implements Observable {
	 ...
	 * plugin manager.
	 * 
	 * If the configuration is overridden than it is in the responsibility of
	 * the caller to execute this method again to apply the new relevant
	...
	public void setup() {
		...
		final String dir = config
				.getStringProperty(ConfigUtils.PLUGINS_DIRECTORY_KEY);
		...
		LOGGER.debug("Starting plugin-scanner timer ({}s)...",
				SCANNER_PERIOD / 1000);
		this.startTimer();
	...
	public PluginInterface getPlugin(final String pluginID, final String version) {
		...
		for (JarPlugin jarPlugin : this.pluginRegistry.values()) {
			...
			if (tmp != null && tmp.getClass().getName().equals(pluginID)
	 ...
	 * all plugins that have the specified name or an empty list. All
	 * comparisons are case insensitive. Note, that all types of plugins are
	 * checked.
	 * 
	 * @param name
	...
	private void startTimer() {
		...
		this.scannerTimer.schedule(new SearchPluginsTask(), new Date(),
				SCANNER_PERIOD);
	}
	 ...
	 * {@inheritDoc}
	 * 
	 * This class always submits itself as a source of the change event and
	 * passes the {@link PluginInfo} of the newly loaded plugin as a message to
	 * all the subscribers.
	 */
	@Override
	public void notifyObservers(ScoutChangeEvent evt) {
		...
		for (final ScoutComponentListener scl : this.observers) {
			try {
				...
				LOGGER.warn("An error occurred, while notifying observers: {}",
						e.getMessage());
			}
		}
	}
	...
	private void setPluginDirectory(final File pluginDirectory) {
		...
		if (pluginDirectory == null) {
			LOGGER.error(
					"Plugin Directory is null. Setting to default directory: {}",
					defDir.getAbsolutePath());
			this.pluginsDirectory = defDir;

		} else if (!pluginDirectory.isDirectory()) {
			LOGGER.error(
					"Plugin Directory is not a directory. Setting to default directory: {}",
					defDir.getAbsolutePath());
			this.pluginsDirectory = defDir;
	...
	private void loadPlugins() {
		...
		final File adaptorPluginDir = new File(this.getPluginDirectory(),
				"adaptors");
		final File notificationPluginDir = new File(this.getPluginDirectory(),
				"notifications");
		...
		if (notificationPluginDir.exists()
				&& notificationPluginDir.isDirectory()) {
	...
	private void loadPlugins(final File dir) {
		...
		for (final File jarFile : jarFiles) {
			if (this.pluginRegistry.containsKey(jarFile)
					&& jarFile.lastModified() == this.pluginRegistry
							.get(jarFile).lastModified) {
				...
				try {
					...
					if (plugin == null) {
						LOGGER.trace(jarFile.getName()
								+ " is not a PluginInterface");
					} else {
						plugin.init();
						synchronized (this.pluginRegistry) {
							this.pluginRegistry.put(jarFile, new JarPlugin(
									plugin, jarFile.lastModified()));
						}

						this.notifyObservers(new ScoutChangeEvent(this,
	...
	private PluginInterface loadPlugin(final File jarFile, final URL[] jarURLs) {
		...
		try {
			...
			LOGGER.error("Could not open jar file '{}': {}", new Object[] {
		...
		while (entries.hasMoreElements()) {
			...
			if (entry.getName().endsWith(CLASS_EXTENSION)) {
				...
				// replacing class path separator
				...
				try {
					...
					if (tmp != null) {
						...
						plugin = tmp;
						LOGGER.info("Plugin {} instantiated", plugin.getName()
								+ ":" + plugin.getVersion());
					}
					...
					LOGGER.error(
							"{}#{} thrown {}: {}",
							new Object[] { jarFile.getName(), className,
		...
		LOGGER.debug("Finished loading jar file: {}", jarFile);
	...
	private PluginInterface createInstance(final Class<?> clazz) {
		...
		try {
			if (PluginInterface.class.isAssignableFrom(clazz)
				...
				LOGGER.info("{} class is a plugin, instantiating",
						clazz.getName());
				plugin = (PluginInterface) clazz.newInstance();
			}

		} catch (final InstantiationException e) {
			LOGGER.error("Could not instantiate class {}: {}", clazz.getName(),
					e.getMessage());
		} catch (final IllegalAccessException e) {
			LOGGER.error("Illegal Access on class {}: {} ", clazz.getName(),
					e.getMessage());
		}
	...
	class SearchPluginsTask extends TimerTask {
		...
		public void run() {
			...
			LOGGER.debug("Search complete - "
					+ PluginManager.this.pluginRegistry.size() + " jar files");

			for (File jarFile : PluginManager.this.pluginRegistry.keySet()) {

				final PluginInterface plugin = PluginManager.this.pluginRegistry
						.get(jarFile).plugin;
