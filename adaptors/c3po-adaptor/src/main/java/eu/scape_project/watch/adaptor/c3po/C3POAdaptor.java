...
import static eu.scape_project.watch.adaptor.c3po.common.C3POConstants.ENDPOINT_TYPE_DESC;
import static eu.scape_project.watch.adaptor.c3po.common.C3POConstants.ENDPOINT_TYPE_DUMMY;
import static eu.scape_project.watch.adaptor.c3po.common.C3POConstants.ENDPOINT_TYPE_FILE;
import static eu.scape_project.watch.adaptor.c3po.common.C3POConstants.ENDPOINT_TYPE_WEBSERVICE;
...
public class C3POAdaptor implements AdaptorPluginInterface {
	...
	public void init() {
		...
		this.commands.put(String.format(CP_DISTRIBUTION, formatDistr),
				new DistributionCommand(formatDistr));
		this.commands.put(String.format(CP_DISTRIBUTION, "Compression scheme"),
				new DistributionCommand(compressionSchemeDistr));
		this.commands.put(String.format(CP_DISTRIBUTION, "Dissimilarities"),
				new DistributionCommand(dissimilaritiesDistr));
	}

	/**
	 * Shuts down the plugin and clears all resources. Warning, reusing the
	 * plugin after a shutdown call (without calling {@link C3POAdaptor#init()}
	 * again) may lead to unexpected behaviour.
	 */
	@Override
	 ...
	 */
	@Override
	public void setParameterValues(final Map<String, String> values)
			throws InvalidParameterException {
	...
	public boolean hasNext() throws PluginException {
		...
		if (this.currentBatch.isEmpty()
				&& this.pointer < this.identifiers.size()) {
	...
	/**
	 * To be removed in version 0.1.0 Fetches all values of all collections in
	 * the pre-configured c3po instance. This method should be used with caution
	 * as it might lead to a huge network overhead, if the c3po instance has a
	 * great deal of collections.
	 * 
	 * @return a {@link ProfileResult} result.
	 * 
	 * @throws PluginException
	 *             if the c3po client cannot be instantiated or another error
	 *             (e.g. network problems) occur.
	 */
	@Override
	@Deprecated
	...
	@Override
	@Deprecated
	public ResultInterface execute(final Map<Entity, List<Property>> context)
			throws PluginException {
		throw new PluginException("This operation is not supported anymore");
	}
	 ...
	 * sleep until it is scheduled again and thus a reset for the next run has
	 * to be done.
	 */
	private void reset() {
	...
	/**
	 * Initializes the default configuration of the adaptor. Currently it starts
	 * a dummy adaptor.
	 */
	private void initConfigs() {
		...
		this.defaultConfig.add(new ConfigParameter(ENDPOINT_CNF, "",
				ENDPOINT_DESC, true));
		this.defaultConfig.add(new ConfigParameter(SCHEDULING_TIME, "60",
				SCHEDULING_TIME_DESC, true));
		this.defaultConfig.add(new ConfigParameter(ENDPOINT_TYPE,
				ENDPOINT_TYPE_WEBSERVICE, ENDPOINT_TYPE_DESC, true));
	 ...
	 * 
	 * @throws PluginException
	 *             if the current configuration is not correct or does not
	...
	private void createSource() throws PluginException {
		...
		String endpointType = this.getParameterValues().get(ENDPOINT_TYPE);

		if (endpointType == null) {
			endpointType = "";
		}

		if (!endpointType.equals(ENDPOINT_TYPE_DUMMY)
				&& StringUtils.isBlank(endpoint)) {
			throw new PluginException("The endpoint value is invalued: "
					+ endpoint);
		}

		if (endpointType.equals(ENDPOINT_TYPE_DUMMY)) {
			this.source = new C3PODummyClient();
		} else if (endpointType.equals(ENDPOINT_TYPE_WEBSERVICE)) {
			this.source = new C3POClient(endpoint);
		} else if (endpointType.equals(ENDPOINT_TYPE_FILE)) {
			this.source = new C3POFileClient(endpoint);
		} else if (endpoint.equals("dummy")) {
			// retro-compatibility
			this.source = new C3PODummyClient();
		...
		if (this.source == null) {
			throw new PluginException(
					"An error occurred, could not initialize a c3po client");
		}
	}

	/**
	 * Gets the values of the given properties for a collection with the given
	 * id. If the list is null or empty, then all known properties will be
	 * fetched. If there is no collection with the given id with the source the
	...
	private List<PropertyValue> getPropertyValues(final String id)
		...
		if (stream != null) {
			final C3POProfileReader reader = new C3POProfileReader(
					this.source.getReader(), stream);
		...
		LOG.debug("Returning property values...");
		LOG.trace("{}", values);
		return values;
	}
