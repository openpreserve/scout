...
public class QuartzScheduler implements SchedulerInterface {
	...
	/**
	 * Default period of monitoring in minutes. This value is used if adaptor
	...
	public void init() {
		try {
			...
			scheduler = new StdSchedulerFactory("config/scout-quartz.properties")
					.getScheduler();
			scheduler.start();
			scheduler.getListenerManager().addJobListener(executionListener,
					EverythingMatcher.allJobs());
			LOG.info("QuartzScheduler initialized");
		} catch (SchedulerException e) {
			LOG.error("An exception occured when initializing QuartzScheduler "
					+ e.getStackTrace());
		}
	...
	public void shutdown() {
		try {
			...
			LOG.error("Failed to shut down QuartzScheduler, an exception occured "
					+ e.getStackTrace());
		}
	}

	@Override
	public void startAdaptor(AdaptorPluginInterface adaptor,
			SourceAdaptorEvent event) {
		...
		if (!adaptorCache.containsAdaptor(adaptor)) {
			...
			if (id != null) {
				...
				String adaptorSchedulePeriod = adaptor.getParameterValues()
						.get("scheduling");
				int iTime = 0;
				...
				try {
					...
					LOG.error("A scheduler exception occurred while starting the adaptor "
							+ adaptor.getName());
					event.setSuccessful(false);
					event.setMessage("Error occurred while starting the adaptor "
							+ adaptor.getName());
					event.setReason("Exception: " + e);
				}

			} else {
				LOG.error("A problem occured when scheduling "
						+ adaptor.getName() + " it is not scheduled!");
				event.setSuccessful(false);
				event.setMessage("A problem occured when scheduling "
						+ adaptor.getName());
	...
	@Override
	public void stopAdaptor(AdaptorPluginInterface adaptor,
			SourceAdaptorEvent event) {
		...
		if (key != null) {
			try {
				...
				LOG.info("Adaptor " + adaptor.getName()
						+ " successfully stopped");
				event.setSuccessful(true);
				event.setMessage("Adaptor " + adaptor.getName()
						+ " successfully stopped");
			} catch (SchedulerException e) {
				LOG.error("A scheduler exception occured while stopping the adaptor "
						+ adaptor.getName());
				event.setSuccessful(false);
	...
	@Override
	public void resumeAdaptor(AdaptorPluginInterface adaptor,
			SourceAdaptorEvent event) {
		...
		if (key != null) {
			try {
				...
				LOG.error("A scheduler exception occured while resuming the adaptor "
						+ adaptor.getName());
				event.setSuccessful(false);
				event.setMessage("Could not resume adaptor "
						+ adaptor.getName());
				event.setReason("Exception: " + e);
			}
		} else {
	...
	@Override
	public void deleteAdaptor(AdaptorPluginInterface adaptor,
			SourceAdaptorEvent event) {
		...
		if (key != null) {
			try {
				...
				LOG.error("A scheduler exception occured while deleting the adaptor "
						+ adaptor.getName());
				event.setSuccessful(false);
				event.setMessage("A scheduler exception occured while deleting the adaptor "
						+ adaptor.getName());
				event.setReason("Exception: " + e);
			}
		} else {
	...
	@Override
	public void executeAdaptor(AdaptorPluginInterface adaptor,
			SourceAdaptorEvent event) {
		...
		if (key != null) {
			try {
				...
				LOG.error("A scheduler exception occured while refiring the adaptor "
						+ adaptor.getName());
				event.setSuccessful(false);
				event.setMessage("A scheduler exception occured while refiring the adaptor "
						+ adaptor.getName());
				event.setReason("Exception: " + e);
			}
		} else {
	 ...
	 */
	public void notifyAdaptorEvent(AdaptorPluginInterface adaptor,
			SourceAdaptorEvent event) {
	...
	public void startRequest(AsyncRequest request) {
		...
		for (final eu.scape_project.watch.domain.Trigger trigger : request
				.getTriggers()) {
	...
	private void startTrigger(eu.scape_project.watch.domain.Trigger trigger,
		...
		if (period > 0) {
			...
			final JobDetail jobDetail = JobBuilder
					.newJob(QuartzRequestTriggerJob.class)
					.withIdentity(id, "triggers").usingJobData("triggerId", id)
					.usingJobData("requestId", request.getId()).build();
			// =======
			//
			// final long period = trigger.getQuestion().getPeriod();
			// // create job detail
			// final JobDetail jobDetail =
			// JobBuilder.newJob(QuartzRequestTriggerJob.class).withIdentity(id,
			// "triggers")
			// .usingJobData("triggerId", id).usingJobData("requestId",
			// request.getId()).build();
			// >>>>>>> improving exception handling in the scheduler ref #126

			// create trigger
			final Trigger qztrigger = TriggerBuilder
					.newTrigger()
					.startNow()
					.withSchedule(
			...
			// schedule it
			try {
				LOG.info("Scheduling trigger {} to run every {}ms",
						new Object[] { id, period });
				...
				LOG.error(
						"A scheduler exception occurred while starting the request trigger {}",
						trigger);
	...
	private void stopTrigger(eu.scape_project.watch.domain.Trigger trigger,
			AsyncRequest request) {
